  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[400px] text-center px-6 py-16">
      ...
      <h2 className="text-2xl font-bold text-white mb-3">
        {tabName} — Plan {meta.name} requerido
      </h2>
      <p className="text-gray-400 mb-2 max-w-md">
        Tu plan actual es <span className="font-semibold text-white capitalize">{PLAN_METADATA[currentPlan].name}</span>.
        Actualiza a <span className={`font-bold ${meta.color}`}>{meta.name}</span> para acceder a esta sección.
      </p>
...
export function LockedSubTab({ requiredPlan, currentPlan }: { requiredPlan: PlanTier; currentPlan: PlanTier }) {
  ...
  return (
    <div className="flex flex-col items-center justify-center min-h-[300px] text-center px-6 py-12">
      ...
      <p className="text-gray-300 font-semibold mb-2">Plan {meta.name} requerido</p>
      <p className="text-gray-500 text-sm mb-6">
        Tu plan actual (<span className="text-white capitalize">{PLAN_METADATA[currentPlan].name}</span>) no incluye esta sección.
      </p>
