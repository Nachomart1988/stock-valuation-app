...
// ── Section definitions ───────────────────────────────────────────────────
interface SectionItem { key: string; label: string; badge?: string }
...
export default function PDFConfigModal({ isOpen, onClose, onGenerate, generating, ticker }: Props) {
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    if (!file.type.startsWith('image/')) { flash('El archivo debe ser una imagen (PNG, JPG, SVG…)'); return; }
    if (file.size > 2 * 1024 * 1024) { flash('La imagen debe ser menor a 2 MB.'); return; }
    try {
      const b64 = await compressLogo(file, 200);
      setLogoBase64(b64);
      setLogoName(file.name);
    } catch { flash('Error al procesar la imagen. Intenta con otro archivo.'); }
    // Reset input so same file can be re-selected
    if (fileRef.current) fileRef.current.value = '';
  };

  // ── Hex color input ────────────────────────────────────────────────────
  const onBgInput = (v: string) => {
    setBgInput(v);
    if (isValidHex(v)) setBgHex(v);
  };
  const onAccentInput = (v: string) => {
    setAccentInput(v);
  ...
  const handleGenerate = async () => {
    if (generating || mlLoading || previewBusy) return;
    let mlPredictions: MLPrediction[] | undefined;
    if (selectedSections.has('ml_predictions')) {
      setMlLoading(true);
      mlPredictions = await fetchML();
      setMlLoading(false);
    }
    const config: PDFConfig = { ...buildBase(), mlPredictions };
    onGenerate(config);
  };
  ...
  const handlePreview = async () => {
    ...
    setPreviewBusy(true);
    let mlPredictions: MLPrediction[] | undefined;
    if (selectedSections.has('ml_predictions')) mlPredictions = await fetchML();
    const config: PDFConfig = { ...buildBase(), mlPredictions, preview: true };
    onGenerate(config);
  ...
  const savePreset = () => {
    ...
    setPresets(updated);
    try { localStorage.setItem(PRESETS_KEY, JSON.stringify(updated)); } catch { /* ignore */ }
    setPresetName('');
  ...
  const busy = generating || mlLoading || previewBusy;
  ...
  return (
    ...
    >
      ...
      <div className="relative w-full max-w-2xl bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col max-h-[92vh]">
        ...
        <div className="flex items-center justify-between px-6 py-4 border-b border-gray-800 shrink-0">
          ...
          <div className="flex items-center gap-2">
            <button
              ...
              aria-label="Gestionar presets guardados"
            >
        ...
        {toast && (
          <div
            className="mx-6 mt-3 px-4 py-2.5 rounded-xl bg-yellow-900/50 border border-yellow-700/60 text-yellow-300 text-sm shrink-0"
            role="alert"
          >
            {toast}
          </div>
        ...
        {showPresets && (
          <div className="mx-6 mt-3 p-4 rounded-xl bg-gray-800/70 border border-gray-700 shrink-0">
            ...
            {presets.length === 0 ? (
              <p className="text-xs text-gray-500 mb-3">No hay presets aún. Guarda tu configuración actual.</p>
            ) : (
                ...
                {presets.map((p, i) => (
                  <div key={i} className="flex items-center gap-2 p-2 rounded-lg bg-gray-900/60">
                    <button
                      ...
                      className="flex-1 text-left text-sm text-gray-200 hover:text-emerald-400 transition truncate"
                    >
                      {p.name}
                    </button>
            ...
            <div className="flex gap-2">
              ...
              <button
                onClick={savePreset}
                className="px-3 py-1.5 rounded-lg bg-emerald-700 hover:bg-emerald-600 text-white text-sm font-medium transition"
              >
                Guardar
              </button>
            </div>
          ...
          <div className="grid sm:grid-cols-2 gap-6">
            ...
            <div>
              <div className="flex items-center justify-between mb-3">
                <h3 className="text-sm font-semibold text-gray-300 uppercase tracking-wider">Secciones</h3>
                <div className="flex gap-3 text-xs">
                  <button
                    onClick={() => setSelectedSections(new Set(ALL_SECTION_KEYS))}
                  ...
                  <button
                    onClick={() => setSelectedSections(new Set(['cover']))}
                    className="text-gray-400 hover:text-red-400 transition"
                    aria-label="Deseleccionar secciones (mantiene mínimo)"
                  >
                    Ninguna
                  </button>
                </div>
              ...
              <div className="space-y-4">
                {SECTION_GROUPS.map(group => (
                  <div key={group.label}>
                    <p className={`text-xs font-semibold uppercase tracking-widest mb-1.5 pl-1 ${group.color}`}>
                      {group.label}
                    </p>
                    <div className="space-y-1.5">
                      {group.sections.map(s => (
                        <label
                          key={s.key}
                          className="flex items-center gap-3 p-2.5 rounded-xl bg-gray-800/50 hover:bg-gray-800 cursor-pointer transition"
                        >
                          <input
                            type="checkbox"
                            ...
                            aria-label={`Incluir sección ${s.label}`}
                          />
                          <span className={`text-sm flex-1 transition ${selectedSections.has(s.key) ? 'text-white' : 'text-gray-500'}`}>
                            {s.label}
                          </span>
                          {s.badge && (
                            <span className="text-xs px-1.5 py-0.5 rounded bg-violet-900/60 text-violet-300 border border-violet-700/60">
                              {s.badge}
                            </span>
                          )}
                        </label>
              ...
              <p className="mt-3 text-xs text-gray-600">
                {selectedSections.size} de {ALL_SECTION_KEYS.length} secciones seleccionadas
              </p>
            </div>
            ...
            <div className="space-y-5">
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    type="color"
                    value={bgHex}
                    onChange={e => { setBgHex(e.target.value); setBgInput(e.target.value); }}
                    className="w-10 h-10 rounded-lg cursor-pointer border border-gray-600 bg-transparent shrink-0"
                  ...
                  <input
                    ...
                    maxLength={7}
                    className={`w-28 px-2.5 py-1.5 rounded-lg text-sm font-mono bg-gray-800 border focus:outline-none ${
                      isValidHex(bgInput) ? 'border-gray-600 text-gray-200' : 'border-red-600 text-red-400'
                    }`}
              ...
              <div>
                ...
                <div className="flex items-center gap-3">
                  <input
                    type="color"
                    value={accentHex}
                    onChange={e => { setAccentHex(e.target.value); setAccentInput(e.target.value); }}
                    className="w-10 h-10 rounded-lg cursor-pointer border border-gray-600 bg-transparent shrink-0"
                  ...
                  <input
                    ...
                    maxLength={7}
                    className={`w-28 px-2.5 py-1.5 rounded-lg text-sm font-mono bg-gray-800 border focus:outline-none ${
                      isValidHex(accentInput) ? 'border-gray-600 text-gray-200' : 'border-red-600 text-red-400'
                    }`}
                    aria-label="Hex del color de acento"
                  />
                  <div
                    className="flex-1 h-2.5 rounded-full"
                    style={{ background: isValidHex(accentHex) ? accentHex : '#00A651' }}
                    aria-hidden="true"
                  />
                </div>
              </div>
              ...
              <div>
                ...
                <textarea
                  ...
                  placeholder="Dejar vacío para usar el disclaimer estándar de Prismo…"
                  rows={3}
                  ...
                  aria-label="Texto del disclaimer personalizado"
                />
              </div>
            </div>
          </div>
        ...
        <div className="flex items-center justify-between px-6 py-4 border-t border-gray-800 bg-gray-900/50 shrink-0">
          ...
          <div className="flex gap-2">
            ...
            <button
              ...
              aria-label="Vista previa del PDF en nueva pestaña"
            >
              {previewBusy ? (
            ...
            <button
              ...
              aria-label="Generar y descargar PDF"
            >
              {(generating || mlLoading) ? (
                <>
