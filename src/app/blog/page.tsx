...
function NewPostForm({ onClose, onSaved }: { onClose: () => void; onSaved: (p: BlogPost) => void }) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-gray-900/60 backdrop-blur-sm p-4">
      <div className="bg-gray-900/80 border border-green-900/20 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        ...
        <div className="p-6 space-y-4">
          <div className="grid grid-cols-2 gap-4">
            <div>
              ...
              <input
                className="w-full bg-gray-900/60 border border-green-900/20 rounded-xl px-4 py-2.5 text-sm font-data uppercase focus:outline-none focus:ring-2 focus:ring-emerald-500"
                placeholder="AAPL"
                value={form.ticker}
            ...
            <div>
              ...
              <select
                className="w-full bg-gray-900/60 border border-green-900/20 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500"
          ...
          <div>
            ...
            <textarea
              rows={5}
              className="w-full bg-gray-900/60 border border-green-900/20 rounded-xl px-4 py-2.5 text-sm focus:outline-none focus:ring-2 focus:ring-emerald-500 resize-none"
        ...
        <div className="flex justify-end gap-3 p-6 border-t border-green-900/20">
          <button onClick={onClose} className="px-5 py-2.5 rounded-xl bg-gray-900/60 hover:bg-gray-900/50 text-sm font-semibold transition">
            Cancelar
          </button>
          <button
            onClick={submit}
...
function PostCard({ post, onClick }: { post: BlogPost; onClick: () => void }) {
  ...
  return (
    <div
      onClick={onClick}
      className="p-5 rounded-2xl bg-gray-900/40 border border-green-900/15 hover:border-emerald-500/40 transition cursor-pointer"
    >
      <div className="flex items-start justify-between gap-3 mb-3">
        <div className="flex items-center gap-2 flex-wrap">
          <span className="font-data font-bold text-white bg-gray-900/50 px-2 py-0.5 rounded text-sm">{post.ticker}</span>
      ...
      <div className="bg-gray-900/80 border border-green-900/20 rounded-2xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <div className="flex items-center justify-between p-6 border-b border-green-900/20">
          <div className="flex items-center gap-2">
            <span className="font-data font-bold text-white bg-gray-900/50 px-2 py-0.5 rounded">{post.ticker}</span>
            <span className={`text-xs px-2 py-0.5 rounded-full border font-semibold ${sentimentColor(post.sentiment)}`}>
              {post.sentiment === 'bullish' ? '📈 Alcista' : post.sentiment === 'bearish' ? '📉 Bajista' : '➖ Neutral'}
            </span>
          </div>
        ...
        <div className="p-6">
          ...
          {(post.targetPrice || post.horizon) && (
            <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 p-4 rounded-xl bg-gray-900/40 border border-green-900/15 mb-6">
              {post.targetPrice && (
                <div>
      ...
      <main className="pt-28 pb-20 px-4 max-w-6xl mx-auto">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-10">
          ...
          <SignedOut>
            <Link
              href="/login"
              className="flex-shrink-0 px-5 py-3 rounded-xl bg-gray-900/50 hover:bg-green-900/15 font-semibold transition text-sm text-center"
            >
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-6">
              <input
                className="bg-gray-900/60 border border-green-900/20 rounded-xl px-4 py-2 text-sm font-data uppercase focus:outline-none focus:ring-2 focus:ring-emerald-500 w-28"
                placeholder="Ticker"
                value={filterTicker}
                ...
                <button
                  ...
                  className={`px-3 py-1.5 rounded-xl text-xs font-semibold transition ${
                    filterSentiment === s
                      ...
                      : 'bg-gray-900/60 text-gray-400 hover:text-white border border-green-900/20'
                  }`}
                >
          ...
          <div className="space-y-6">
            {/* How it works */}
            <div className="p-5 rounded-2xl bg-gray-900/40 border border-green-900/15">
