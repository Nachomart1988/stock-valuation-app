...
export async function GET(req: NextRequest) {
  ...
  const minGap = parseFloat(sp.get('minGap') || '0.15');
  const lookbackDays = parseInt(sp.get('lookbackDays') || '504');
  ...
  // 2. Scan each stock for Episodic Pivots via backend
  const results: any[] = [];
  let scanned = 0;

  const scanEP = async (stock: any) => {
    try {
      const res = await fetch(`${backendUrl}/ep/detect`, {
        method: 'POST',
      ...
      const data = await res.json();
      scanned++;
      // Accept any result with episodes found (score > 0), not just detected=true
      if (data.score > 0 && data.episodes?.length > 0) {
        results.push({
          symbol: stock.symbol,
          ...
          bestEpisode: data.best_episode || null,
  ...
  for (let i = 0; i < valid.length; i += SCAN_CONCURRENCY) {
    ...
    await Promise.all(wave.map(scanEP));
  }
